...
window.cadastrarVeiculo = async function() {
    ...
    if (!prefixo || !placa) {
        alert("Preencha os campos obrigatórios!"); s
        return;
    }
    const idDoTipo = parseInt(tipoId);
    const payload = {
        prefixo: prefixo,
        placa: placa,
        tipo: {
    ...
    try {
        ...
        if (response.ok) {
            ...
            ["cad-marca", "cad-modelo", "cad-tipo", "cad-placa", "cad-prefixo"].forEach(id => {
                document.getElementById(id).value = "";
            });
        } else {
            const erroMsg = await response.text();
            alert("Erro ao cadastrar veículo: " + erroMsg);
        }
    } catch (e) {
